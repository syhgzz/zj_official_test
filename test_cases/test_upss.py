...
def test_get_overview(client: APIClient):
    ...
    print_response('获取沉降态势模块概览', 'GET', '/api/v1/upss/overview', response, config.verbose)
...
def test_get_periods(client: APIClient):
    ...
    print_response('获取沉降期次列表', 'GET', '/api/v1/upss/periods', response, config.verbose)
    if config.save_response and response:
...
def test_get_period_summary(client: APIClient, issue: str = "20221220"):
    ...
    print_response('获取期次汇总统计', 'GET', f'/api/v1/upss/periods/{issue}/summary', response, config.verbose)
    if config.save_response and response:
...
def test_get_point_history(client: APIClient, code: str = "PS001"):
    ...
    print_response('获取单点沉降历史', 'GET', f'/api/v1/upss/points/{code}/history', response, config.verbose)
    if config.save_response and response:
...
def test_get_regional_statistics(client: APIClient):
    ...
    print_response('获取区域沉降统计', 'GET', '/api/v1/upss/statistics/regional', response, config.verbose)
    if config.save_response and response:
...
def test_get_grid_rate(client: APIClient):
    ...
    print_response('获取网格沉降速率', 'GET', '/api/v1/upss/statistics/gridRate', response, config.verbose)
    if config.save_response and response:
...
def test_get_grid_gradient(client: APIClient):
    ...
    print_response('获取网格沉降梯度', 'GET', '/api/v1/upss/statistics/gridGradient', response, config.verbose)
    if config.save_response and response:
...
def test_get_warning_issue(client: APIClient):
    ...
    print_response('获取沉降预警信息', 'GET', '/api/v1/upss/visualization/warning/issue', response, config.verbose)
    if config.save_response and response:
...
def test_get_statistics_issue(client: APIClient):
    ...
    print_response('获取沉降态势统计', 'GET', '/api/v1/upss/visualization/statistics/issue', response, config.verbose)
    if config.save_response and response:
...
def test_get_max_subsidence_timeseries(client: APIClient):
    ...
    print_response('获取最大沉降点时序', 'GET', '/api/v1/upss/visualization/max-subsidence/timeseries', response, config.verbose)
    if config.save_response and response:
...
def test_get_top_gradient(client: APIClient):
    ...
    print_response('获取Top5沉降梯度', 'GET', '/api/v1/upss/visualization/top-gradient', response, config.verbose)
    if config.save_response and response:
