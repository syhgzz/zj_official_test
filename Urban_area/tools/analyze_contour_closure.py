# -*- coding: utf-8 -*-
"""
analyze_contour_closure.py —— 从入口方法出发算调用闭包，用来确定"到底要移植哪些方法"。

当初就是靠它发现 `tracingPolygons(LineList, borderPoint, Extent, contour)`（L3046-3479）
只被 createContourPolygons / createCutContourPolygons 调用、不在参考链路上，从而省掉 434 行移植。

用法：python analyze_contour_closure.py [Contour.java 路径]
默认读随包携带的上游源码 wcontour/_upstream/Contour.java。
"""
import os
import re
import sys
...
_DEFAULT_SRC = os.path.join(os.path.dirname(_HERE), 'wcontour', '_upstream', 'Contour.java')
...
json.dump([{k: v for k, v in d.items() if k in ('name', 'args', 'ret', 'lineno', 'endline')}
          ...
          open(os.path.join(_HERE, 'contour_closure.json'), 'w', encoding='utf-8'),
